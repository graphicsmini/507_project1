...
# Ensure each of your subclasses have the class variable(s), constructor and string method they need. You will need to infer from reading the code in the class Currency what you need in order to correctly implement the subclasses.
...
# fill in code for class definition
...
class Bank:
    def __init__(self, name, currency_name, initial_value=0):
        self.name = name
        self.unit = currency_name
        self.current_account = self.unit(initial_value)

    def __str__(self):
        return "Welcome to the {} bank! {} Bank holds the {} currency and currently holds {} of {}".format(self.name, self.name, self.unit.unit_name, self.current_account.value, self.unit.unit_name)

    def deposit(self, instance_of_current_name):
        if isinstance(instance_of_current_name, self.unit):
            self.current_account.value = self.current_account.value + instance_of_current_name.value
